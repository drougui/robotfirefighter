...
let server = mynet.createServer(onClientConnected);
server.listen(myPORT, myADDRESS);
var m
...
console.log(`Server started at: ${myADDRESS}:${myPORT}`);


router.get('/AOI', function(req, res) {
	res.json(m);
	m = -1;
});
*/
...
// NEW SAMPLING ALARMS TIMEOUTS & VARS
var alarmLowbatteryTimeout;
var alarmTemperatureTimeout;
var alarmAutonomousTimeout;
var alarmRemainingtimeTimeout;
var alarmManualTimeout;
var currentManualTime = 0;
var currentKeysNotUsedTime = 0;
var alarmWaterlevelcontainerTimeout; 
...
//      currentPseudo = '';
      ...
      clearTimeout(alarmLowbatteryTimeout);
      clearTimeout(alarmTemperatureTimeout);
      clearTimeout(alarmAutonomousTimeout);
      clearTimeout(alarmRemainingtimeTimeout);
      clearTimeout(alarmManualTimeout);
      currentManualTime = 0;
      currentKeysNotUsedTime = 0;
      clearTimeout(alarmWaterlevelcontainerTimeout); 
    ...
    if(decoded.auth && decoded.exp === global.expires){
      if(req.body.key == 'front') {
        ...
        if(autonomousRobot==0){
          if(currentAutoMvt && !beginning){
            // make sure autonomy terminated
            var udpMessAbort = abortMoveToUdpMess();
            var bufferAbort = new Buffer(udpMessAbort);
            var clientAbort = dgram.createSocket('udp4');
            clientAbort.send(bufferAbort, 0, bufferAbort.length, PORT, HOST, function(err) {
              if(err) throw err;
...
*/          prevAutonomousRobot = autonomousRobot;
...
// ON PEUT METTRE LE CLEAR ET LE SITUATION=FALSE la ou ils sont actuellement
//  qd pas la condition
// qd deja lancer 

  // TODO put every alarms here (in this interval, freq = 1sec!)
  // FOR THAT WE NEED TO DECLARE 1 TIMEOUT FOR EACH ALARM
  // CONDITION => launch timeout (in this interval)
  // !CONDITION => kill it
  // TODO ALARMES EN FRANCAIS SELON ROOTSCOPE.LANGUAGE
  //=====================================================
  // remaining time part
  //=====================================================
      timeInterval = setInterval(function() {
        if(remainingtime<=550){
          beginning = false;
        }
        if(remainingtime > 0) {
          ...
          // === ALARMS ===
          // ==============
          // TODO VIRER LES AUTRES ALARMES
          // TODO TESTER
          // utiliser stats.py pour valider le sampling
          // TODO a definir en global et clear a l'init et clear a la fin
          // TODO verifier que cela correspond aux conditions de stats.py
          /*
          var alarmLowbatteryTimeout;
          var alarmTemperatureTimeout;
          var alarmAutonomousTimeout;
...
*/
          ...
          // | low battery |
          // ---------------
          // when the condition is not valid anymore, be sure the timeout is cleared 
          if(batteryLevel - 10 > Math.sqrt( Math.pow(zoneslocations[1].x-robotx, 2) + Math.pow(zoneslocations[1].y-roboty[0], 2) )*0.6){
            clearTimeout(alarmLowbatteryTimeout);
            alarmSituations[0] = false;
          }
          // displayed with prob 0.5, with a delay Unif([0,9]) 
          if(batteryLevel - 10 <= Math.sqrt( Math.pow(zoneslocations[1].x-robotx, 2) + Math.pow(zoneslocations[1].y-roboty[0], 2) )*0.6 && !alarmSituations[0]){
            alarmSituations[0] = true;
            if(Math.random()>0.5){
              alarmLowbatteryTimeout = setTimeout(function() {
                if(!alarmOverlayOpen){
                  alarmOverlayOpen = true;
                  alarmCause = 0;
                  writeAlarms.push(alarmCause);
          ...
          if(mercurelevelfloat>200 && !alarmSituations[1]){
            alarmSituations[1] = true;
            if(Math.random()>0.5){
              alarmTemperatureTimeout = setTimeout(function() {
                if(!alarmOverlayOpen){
                  alarmOverlayOpen = true;
                  alarmCause = 1;
                  writeAlarms.push(alarmCause);
          ...
          if(remainingtime == 60 && Math.random()>0.5){
            alarmRemainingtimeTimeout = setTimeout(function() {
              if(!alarmOverlayOpen){
                alarmOverlayOpen = true;
                alarmCause = 2;
                writeAlarms.push(alarmCause);
          ...
          // we do not need to kill the timeout here since the condition cannot end (<=60)
          // we do not need the alarmSituations[2] since it happens only once

          // --------------
          // | waterlevel |
          // --------------
          // displayed with prob 0.5 (1sec event)
          if(watlevel>20 || watlevelTemp<=20){
          ...
          // we do not need a timeout here since the condition does not exceed 1sec


          // for auto/manual alarms
          var keysUsed = writeUsedKeys.length!=0;
          // ---------------
          // | autonomous  |
          // ---------------
          // when the condition is not valid anymore, be sure the timeout is cleared 
          if(autonomousRobot==0 || !keysUsed){
            clearTimeout(alarmAutonomousTimeout);
            alarmSituations[4] = false;
          }
          // displayed with prob 0.5, with a delay Unif([0,9])
          if(autonomousRobot==1 && keysUsed && !alarmSituations[4]){
            alarmSituations[4] = true;
            if(Math.random()>0.5){
              alarmAutonomousTimeout = setTimeout(function() {
                if(!alarmOverlayOpen){
                  alarmOverlayOpen = true;
                  alarmCause = 4;
                  writeAlarms.push(alarmCause);
          ...
          // when the condition is not valid anymore, be sure the timeout is cleared 
          if(autonomousRobot==1 || currentManualTime>currentKeysNotUsedTime+1){
            clearTimeout(alarmManualTimeout);
            alarmSituations[5] = false;
          }
          // displayed with prob 0.5, with a delay Unif([0,9])
          if(autonomousRobot==0 && currentManualTime<=currentKeysNotUsedTime+1 && !alarmSituations[5]){
            alarmSituations[5] = true;
            if(Math.random()>0.5){
              alarmManualTimeout = setTimeout(function() {
                if(!alarmOverlayOpen){
                  alarmOverlayOpen = true;
                  alarmCause = 5;
                  writeAlarms.push(alarmCause);
          ...
          // -----------------------
          // when the condition is not valid anymore, be sure the timeout is cleared 
          if(watlevelContainer>=15){
            clearTimeout(alarmWaterlevelcontainerTimeout);
            alarmSituations[7] = false;
          }
          // displayed with prob 0.5, with a delay Unif([0,9])
          if(watlevelContainer<15 && !alarmSituations[7]){
            alarmSituations[7] = true;
            if(Math.random()>0.5){
              alarmWaterlevelcontainerTimeout = setTimeout(function() {
                if(!alarmOverlayOpen){
                  alarmOverlayOpen = true;
                  alarmCause = 7;
                  writeAlarms.push(alarmCause);
